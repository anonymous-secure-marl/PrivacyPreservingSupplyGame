    ...
    def update(self, g, gg, policies, retail_market, raw_mat_cost, step, 
        ...
        for p_ind in range(self.player_num):
            ...
            if p_ind==0:
                p0 = self.players[0]
                state0 = p0.obs.copy()
    #             print("state 0 print start for it " + str(step) + "\n")
                f = open("state0.txt","w")
                for s in state0:
                    f.write(str(s))
                    f.write("\n")
                for i in range(5):
                    ...
                    f.write("\n")
                f.close()
#             print("state 0 print end for it " + str(step) + "\n")
            #################################
            ...
            if hasattr(policies, 'actor'):
                ...
                if p_ind==0:
#                     print("0 in 0 start\n")
#                     s0In0 = self.players[0].obs.copy()
#                     print(s0In0)
#                     s1In0 = self.players[1].obs.copy()
#                     print(s1In0)
                    
                    command = "cat state0.txt wNb_Player0Actor.txt action_lim_high_batch1.txt | ../../../EzPC/SCI/vital/build/ActorForward_1batch0 r=1 add=10.0.0.5 > action0.txt; sed -i '13,14!d ; s/^.*: //g' action0.txt"

                    subprocess.call(command, shell=True)
                    
#                     print("0 in 0 end\n")
                    
#                     print("Read from action0 starts\n")
                    
                    p_action_list=[]
                    f = open("action0.txt","r")
                    for i in range(2):
                        x = float(f.readline())
...
#                     print(s1In0)
                    ...
                    command = "cat state0.txt | ../../../EzPC/SCI/vital/build/ActorForward_1batch0 r=2 add=10.0.0.5 > /dev/null"

                    subprocess.call(command, shell=True)
                    
#                     print("1 in 0 end\n")
                    
#                     print("Simulation for 1 in 0 starts\n")
...
#                     print("Simulation for 1 in 0 ends\n")
            else:
...
#             print("After Normalization")
            ...
            if self.graph.in_degree(p_ind) == 0:
                ...
                for s_ind in self.graph.predecessors(p_ind):
                    ...
                    ###################################################
                    time.sleep(3)
                    f = open("Demand_1to0.txt","r")
                    x = float(f.readline())
                    f.close()
                    demand[p_ind, s_ind] = x
            ...
            if self.graph.out_degree(p_ind) == 0: # market facing retailer
                ...
                for r_ind in self.graph.successors(p_ind):
                    ...
                    f = open("Price_0to1.txt","w")
                    f.write(str(cost[next_retailer]))
                    f.close()
                    subprocess.run(["scp", "Price_0to1.txt", "VM1_Name@10.0.0.5:/home/VM1_Name/UAI_rebuttal/Supply-Chain/marl"])
                    time.sleep(3)
                    ###################################################
                    next_retailer += 1
            #########################################        
        ...
        for p_ind in range(self.player_num):
            ...
            if self.graph.out_degree(p_ind) != 0: # not retail market facing
                ...
                for r_ind in self.graph.successors(p_ind):
                    ...
                    f = open("Sold_0to1.txt","w")
                    f.write(str(prod_sold[next_retailer]))
                    f.close()
                    subprocess.run(["scp", "Sold_0to1.txt", "VM1_Name@10.0.0.5:/home/VM1_Name/UAI_rebuttal/Supply-Chain/marl"])
                    time.sleep(3)
                    ###################################################
                    next_retailer += 1
            ###########################################
