...
#                     for p in range(len(optimizers))]
                    ...
                    print("ddpg 0in0 starts")
                    p=0
                    ddpg.update_target0in0(SampleBatchByPlayer0, SampleBatchByPlayer0_c,
                        ac_nns, optimizers[p], memories, batch_size,
                        ac_fig.obs_inds, ac_fig.a_inds[p], r_inds=p, 
                        p_ind=p, actor_state_mode=ac_fig.actor_mode,
                        critic_action_mode=ac_fig.critic_action_mode,
                        critic_state_mode=ac_fig.critic_state_mode,
                        p_graph=world.graph, slice_graph=ac_fig.slice_graph)
                    print("ddpg 0in0 ends")
                    
                    print("ddpg 1in0 starts")
                    p=1
                    ddpg.update_target1in0(SampleBatchByPlayer1, SampleBatchByPlayer1_c,
                        ac_nns, optimizers[p], memories, batch_size,
                        ac_fig.obs_inds, ac_fig.a_inds[p], r_inds=p, 
                        p_ind=p, actor_state_mode=ac_fig.actor_mode,
                        critic_action_mode=ac_fig.critic_action_mode,
...
#                     print("YESSSSSSSSSSSSSSSSSSSSS")
                    ...
                    actor_targ_params = []
                    f = open("wNb_Player0Actor_targ.txt","r")
                    for i in range(18178):
                        x = float(f.readline())
                    ...
                    critic_params = []
                    f = open("wNb_Player0Critic.txt","r")
                    ...
                    critic_targ_params = []
                    f = open("wNb_Player0Critic_targ.txt","r")
                    for i in range(18561):
                        x = float(f.readline())
