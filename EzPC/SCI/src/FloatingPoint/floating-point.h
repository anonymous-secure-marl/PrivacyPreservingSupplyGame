...
public:
  ...
  FPArray vector_sum_(const vector<FPArray> &x, int b_, int sc, int m_bits, int e_bits);
  FPArray vector_sum_(const vector<FPArray> &x) {
    int m_bits = x[0].m_bits;
    int e_bits = x[0].e_bits;
    return vector_sum_(x, m_bits, m_bits, m_bits, e_bits);
  }

  FPArray vector_sum(vector<FPArray> &x, int b_, int sc, int m_bits, int e_bits);
  FPArray vector_sum(vector<FPArray> &x) {
    int m_bits = x[0].m_bits;
    int e_bits = x[0].e_bits;
    return vector_sum(x, m_bits, m_bits, m_bits, e_bits);
  }

  FPArray treesum(const vector<FPArray> &x) ;

  FPArray dot_product(const vector<FPArray> &x, const vector<FPArray> &y, bool impure = true);

  vector<FPArray> enlist_products(const FPMatrix &x, const FPMatrix &y, bool impure = true) ;

  vector<FPArray> enlist_products_baseline(const FPMatrix &x, const FPMatrix &y) ;

  FPMatrix matrix_multiplication(const FPMatrix &x, const FPMatrix &y, bool impure = true) ;

  FPMatrix matrix_multiplication_baseline(const FPMatrix &x, const FPMatrix &y) ;

  vector<FPMatrix> matrix_multiplication(const vector<FPMatrix> &x, const vector<FPMatrix> &y, bool impure = true) ;

  vector<FPMatrix> matrix_multiplication_baseline(const vector<FPMatrix> &x, const vector<FPMatrix> &y) ;

  FPArray bfloat16_to_FP32(const FPArray &x);
  FPArray FP32_to_bfloat16(const FPArray &x);

  FPArray div_softfloat(const FPArray &x, const FPArray &y, bool check_bounds = true);
  FPArray add_softfloat(const FPArray &x, const FPArray &y);
  FPArray mul_softfloat(const FPArray &x, const FPArray &y);
  BoolArray LT_softfloat(const FPArray &x, const FPArray &y);
};
