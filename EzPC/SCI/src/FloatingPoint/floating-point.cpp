...
#define CHUNK_SIZE_BASELINE (1 << 15)
...
void FPOp::round_and_check(FixArray &m, FixArray &e, int s) {
  ...
  FixArray m_if = fix->round_nearest(m, s);
...
FPArray FPOp::add(const FPArray &x, const FPArray &y, bool cheap_variant,
  ...
  // float comm_start = iopack->get_comm() ;
  ...
  // float comm_end = iopack->get_comm() ;

  // printf("Before normalization - \n") ;
  // printf("\tTime = %f\n", t/1000.0) ;
  // printf("\tComm = %f\n", (comm_end - comm_start)/(1<<20)) ;
...
FPArray FPOp::vector_sum_(const vector<FPArray> &x, int b_, int sc, int m_bits, int e_bits) {
  int N = x.size();
  int n = x[0].size;
  int b = x[0].m_bits + 1;
  assert(m_bits > 0);
...
FPArray FPOp::vector_sum(vector<FPArray> &x, int b_, int sc, int m_bits, int e_bits) {
  int N = x.size();
  uint64_t n = x[0].size;
  assert(m_bits > 0);
  int b = x[0].m_bits + 1;
  ...
  if (num_chunks == 1) {
    return vector_sum_(x_slices, b_, sc, m_bits, e_bits);
  } else {
    FPArray sum_tr;
    if (num_chunks * chunk_size == n) {
      sum_tr = vector_sum_(x_slices, b_, sc, m_bits, e_bits);
    } else {
      ...
      FPArray sum_tr_1 = vector_sum_(x_slices_1, b_, sc, m_bits, e_bits);
      FPArray sum_tr_2 = vector_sum_(x_slices_2, b_, sc, m_bits, e_bits);
    ...
    return vector_sum(summ, m_bits, m_bits, m_bits, e_bits) ;
  }
}

/*
FPArray FPOp::vector_sum_(const vector<FPArray> &x, bool neg_terms, int extra_m_bits, int extra_m_scale) {
  int N = x.size();
  int n = x[0].size;
  int m_bits = x[0].m_bits - extra_m_bits;
  int e_bits = x[0].e_bits;
  assert(m_bits > 0);
  int logn = ceil(log2(n));
  int extra_thrshld = logn;

  assert(2*m_bits + logn + extra_m_bits + extra_thrshld + 3 < 64) ;

  // These arrays store the transposed version (nxN) of x (Nxn)
  // if we use get_components on elements of x, x_m will have scale m_bits + extra_m_bits, whereas it should be m_bits
  FixArray x_m = fix->input(this->party, N*n, uint64_t(0), false, m_bits + 1 + extra_m_bits, m_bits + extra_m_scale);
  FixArray x_e = fix->input(this->party, N*n, uint64_t(0), true, e_bits + 2, 0);
  BoolArray x_s = bool_op->input(this->party, N*n, uint8_t(0));
  ...
  FixArray e_max_concat = fix->max(e_concat);
  FixArray e_thr;
  e_thr = fix->sub(e_max_concat, m_bits + extra_thrshld + 1);
  ...
  BoolArray all_0 = bool_op->input(ALICE, m.size, uint8_t(0));
  FixArray shift_amt = fix->sub(x_e, e_thr_copy); shift_amt.signed_ = false;
  // m = fix->left_shift(fix->extend(m, m_bits + logn + extra_m_bits + 2, bool_op->NOT(e_lt_e_thr).data), shift_amt,
  m = fix->left_shift(fix->extend(m, m_bits + logn + extra_m_bits + 2), shift_amt,
      2*m_bits + logn + extra_m_bits + extra_thrshld + 3, m_bits + extra_thrshld + 1, all_0.data);
  m = fix->if_else(x_s, fix->mul(m, -1), m);

  FixArray M; FixArray e;
  M = fix->input(this->party, N, uint64_t(0), m.signed_, m.ell, m.s);
  e = e_thr;
  for(int i = 0; i < n; i++) {
    for(int j = 0; j < N; j++) {
      M.data[j] += m.data[i*N + j];
  ...
  tie(s, z) = fix->LT_and_EQ(M, 0);
  M = fix->if_else(s, fix->mul(M, -1), M);
  this->normalize(M, e, m_bits + extra_m_scale, 2*m_bits + logn + extra_m_bits + extra_thrshld + 2);
  M = fix->reduce(M, 2*m_bits + logn + extra_m_bits + extra_thrshld + 2);
  fp_op->round_and_check(M, e, m_bits + logn + extra_m_bits + extra_thrshld + 1);
  FPArray alpha = this->input(this->party, N, s.data, z.data, M.data, e.data, m_bits, e_bits);
  return this->check_bounds(alpha, true);
}

FPArray FPOp::vector_sum(vector<FPArray> &x, bool neg_terms, int extra_m_bits, int extra_m_scale) {
  int N = x.size();
  uint64_t n = x[0].size;
  int m_bits = x[0].m_bits - extra_m_bits;
  int e_bits = x[0].e_bits;
  ...
  for(int i = 0; i < N; i++) {
    assert(x[i].party != PUBLIC);
    assert(x[i].m_bits == m_bits + extra_m_bits);
    assert(x[i].e_bits == e_bits);
    assert(x[i].size == n);
  }
  int logn = ceil(log2(n));
  ...
  // assert(2*m_bits + logn + extra_m_bits + extra_thrshld + 3 < 64)
  int chunk_size = 1 << logn;
  if (extra_m_bits + extra_thrshld + logn + 2*m_bits > 60) {
    do {
      chunk_size /= 2;
      extra_thrshld = ceil(log2(chunk_size));
    } while (extra_m_bits + extra_thrshld + ceil(log2(chunk_size)) + 2*m_bits > 60);
  }
  int num_chunks = ceil(n/double(chunk_size));
  ...
  if (num_chunks == 1) {
    return vector_sum_(x_slices, neg_terms, extra_m_bits, extra_m_scale);
  } else {
    FPArray sum_tr;
    if (num_chunks * chunk_size == n) {
      sum_tr = vector_sum_(x_slices, neg_terms, extra_m_bits, extra_m_scale);
    } else {
      vector<FPArray> x_slices_1(N*(num_chunks - 1));
      vector<FPArray> x_slices_2(N);
      for (int j = 0; j < num_chunks - 1; j++) {
      ...
      FPArray sum_tr_1 = vector_sum_(x_slices_1, neg_terms, extra_m_bits, extra_m_scale);
      FPArray sum_tr_2 = vector_sum_(x_slices_2, neg_terms, extra_m_bits, extra_m_scale);
      sum_tr = concat({ sum_tr_1, sum_tr_2 });
    }
    vector<FPArray> summ(N);
    for (int i = 0; i < N; i++) {
    ...
    return vector_sum(summ, neg_terms, 0, 0) ;
  }
}
*/

FPArray FPOp::dot_product(const vector<FPArray> &x, const vector<FPArray> &y, bool impure) {
  int N = x.size();
  int n = x[0].size;
  int m_bits = x[0].m_bits;
  int e_bits = x[0].e_bits;
  ...
  int b, b_, sc;
  if (impure &&
      (m_bits == BFLOAT16_M_BITS && e_bits == BFLOAT16_E_BITS)
  ...
  // return vector_sum(prod, true, extra_m_bits, extra_m_scale);
  return vector_sum(prod, b_, sc, m_bits, e_bits);
}

vector<FPArray> FPOp::enlist_products(const FPMatrix &x, const FPMatrix &y, bool impure) {
  ...
  BoolArray prod_zx = bool_op->input(this->party, x.dim1*x.dim2, uint8_t(0));
  BoolArray prod_zy = bool_op->input(this->party, y.dim1*y.dim2, uint8_t(0));
  ...
  FixArray prod_m_ = fix->input(this->party, N*n, uint64_t(0), false, 2*m_bits + 2, 2*m_bits);
  FixArray prod_e = fix->input(this->party, N*n, uint64_t(0), true, e_bits + 2, 0);
  BoolArray prod_s = bool_op->input(this->party, N*n, uint8_t(0));
...
vector<FPArray> FPOp::enlist_products_baseline(const FPMatrix &x, const FPMatrix &y) {
  assert(x.party != PUBLIC); assert(y.party != PUBLIC);
  assert(x.dim2 == y.dim1);
  assert(x.m_bits == y.m_bits); assert(x.e_bits == y.e_bits);
  ...
  for (int i = 0 ; i < x.dim1 ; i++) {
    for (int j = 0 ; j < y.dim2 ; j++) {
      int ind = i*y.dim2 + j ;
      prod_x[ind] = FPArray(this->party, n, m_bits, e_bits) ; 
      prod_y[ind] = FPArray(this->party, n, m_bits, e_bits) ;
  ...
  return this->mul(prod_x, prod_y) ;
}

FPMatrix FPOp::matrix_multiplication(const FPMatrix &x, const FPMatrix &y, bool impure) {
  assert(x.party != PUBLIC); assert(y.party != PUBLIC);
  assert(x.dim2 == y.dim1);
  assert(x.m_bits == y.m_bits); assert(x.e_bits == y.e_bits);
  ...
  vector<FPArray> prod = enlist_products(x, y, impure) ;
  ...
  for (int i = 0; i < N; i += rows_per_batch) {
    ...
    FPArray ret_i = vector_sum(prod_i, b_, sc, m_bits, e_bits);
...
FPMatrix FPOp::matrix_multiplication_baseline(const FPMatrix &x, const FPMatrix &y) {
  assert(x.party != PUBLIC); assert(y.party != PUBLIC);
  assert(x.dim2 == y.dim1);
  assert(x.m_bits == y.m_bits); assert(x.e_bits == y.e_bits);
  ...
  // return FPMatrix(x.dim1, y.dim2, treesum(enlist_products_baseline(x, y)));

  int N = x.dim1*y.dim2; int n = x.dim2;
  int m_bits = x.m_bits; int e_bits = x.e_bits;
  
  vector<FPArray> prod = enlist_products_baseline(x, y) ;
  FPMatrix ret(this->party, x.dim1, y.dim2, m_bits, e_bits) ;
  int rows_per_batch = ceil(CHUNK_SIZE_BASELINE/double(n));

  // long long t = time_from(start);
  // float comm_end = iopack->get_comm() ;

  // printf("Before treesum - \n") ;
  // printf("\tTime = %f\n", t/1000.0) ;
  // printf("\tComm = %f\n", (comm_end - comm_start)/(1<<20)) ;
...
vector<FPMatrix> FPOp::matrix_multiplication(const vector<FPMatrix> &x, const vector<FPMatrix> &y, bool impure) {
  int m, n, p, L ;
  int m_bits, e_bits ;
  m = x[0].dim1 ;
  n = x[0].dim2 ;
  ...
  vector<FPArray> prod = enlist_products(x_concat, y_concat, impure) ;
  assert(prod[0].m_bits == b-1);
  ...
  for (int l = 0 ; l < L ; l++) {
    ...
    for (int i = 0 ; i < m*p ; i += rows_per_batch) {
      ...
      FPArray summed = vector_sum(tosum, b_, sc, m_bits, e_bits) ;
...
vector<FPMatrix> FPOp::matrix_multiplication_baseline(const vector<FPMatrix> &x, const vector<FPMatrix> &y) {
  int m, n, p, L ;
  int m_bits, e_bits ;
  m = x[0].dim1 ;
  n = x[0].dim2 ;
  ...
  vector<FPArray> prod = enlist_products_baseline(x_concat, y_concat) ;
  int rows_per_batch = ceil(CHUNK_SIZE_BASELINE/double(n));
  vector<FPMatrix> ret ;
  for (int l = 0 ; l < L ; l++) {
    FPMatrix ret_l(this->party, m, p, m_bits, e_bits) ;
