...
#define M_BITS_EXP_IMPRECISE 32
#define PI_DOUBLE                                                              \
...
FPArray FPMath::exp_neg(const FPArray &x, uint8_t max_ULP_error) {
  assert(x.party != PUBLIC);
  ...
  // bfloat16 special case (as precise as it gets)
  if (x.m_bits == 7 && x.e_bits == 8) {
    x_e.signed_ = false;
    // only 4 bits of exponent are needed
    FixArray idx_hi = fix->scale_up(fix->reduce(fix->add(x_e, 9), 4), x.m_bits + 4, x.m_bits);
    FixArray idx_lo = fix->extend(fix->sub(x_m, 1ULL << x.m_bits), x.m_bits + 4);
    FixArray idx = fix->add(idx_hi, idx_lo);

    // all outputs are positive, so ignoring sign bit
    FixArray y_int = fix->LUT(neg_exp_bfloat16, idx, false, 15, x.m_bits);
    ...
    BoolArray cond1 = fp_op->LT(x, 93.0);
    BoolArray cond2 = fix->GE(x_e, -9 + x.e_bias());

    y = fp_op->if_else(cond1, y, 0.0);
    y = fp_op->if_else(cond2, y, 1.0);
    return y;
  }
  ...
  BoolArray x_ge_LOGE2 = fp_op->GE<double>(x, double(LOGE2));
  FixArray shift_amt = fix->add(x_e, 1 - x.e_bias());
  shift_amt.signed_ = false;
  FixArray m = fix->left_shift(x_m, shift_amt, x.m_bits + 10, 9, all_1.data);
  m.s += 1;
  FixArray N = fix->mul(m, uint64_t(LOG2E * (1ULL << FP_INTMD_M_BITS)),
                        x.m_bits + FP_INTMD_M_BITS + 11, all_0.data);
  N.s += FP_INTMD_M_BITS;
  N = fix->truncate_reduce(N, x.m_bits + 1 + FP_INTMD_M_BITS);

  FixArray f = fix->mul(N, uint64_t(LOGE2 * (1ULL << (M_BITS_EXP_IMPRECISE))),
                        M_BITS_EXP_IMPRECISE + 1, all_0.data);
  f.s = M_BITS_EXP_IMPRECISE;
  f = fix->sub(fix->scale_up(m, M_BITS_EXP_IMPRECISE + 1, M_BITS_EXP_IMPRECISE), f);
  BoolArray zero_f = fix->EQ(f, 0);
  N = fix->mul(N, -1);
  N.signed_ = true;

  FixArray delta_m = f;
  FixArray delta_e = fix->input(PUBLIC, x.size, uint64_t(0), x_e.signed_, x_e.ell, x_e.s);
  fp_op->normalize(delta_m, delta_e, M_BITS_EXP_IMPRECISE - x.e_bias());
  delta_m = fix->truncate_reduce(delta_m, M_BITS_EXP_IMPRECISE - FP_INTMD_M_BITS);
  delta_e = fix->if_else(zero_f, 0, delta_e);
  FPArray delta = fp_op->input(x.party, x.size, all_0.data, zero_f.data,
          delta_m.data, delta_e.data, FP_INTMD_M_BITS, FP_INTMD_E_BITS);
  ...
  FixArray x_m_prec = fix->scale_up(x_m, FP_INTMD_M_BITS + 1, FP_INTMD_M_BITS);
  FixArray x_e_prec = x_e;
  FPArray x_prec = fp_op->input(x.party, x.size, all_0.data, x_z.data,
          x_m_prec.data, x_e_prec.data, FP_INTMD_M_BITS, FP_INTMD_E_BITS);
  N = fix->if_else(x_ge_LOGE2, N, 0);
  delta = fp_op->if_else(x_ge_LOGE2, delta, x_prec);
  BoolArray delta_s, delta_z;
  tie(delta_s, delta_z, delta_m, delta_e) = fp_op->get_components(delta);
  ...
  // TODO
  // else evaluate spline
  vector<vector<FPArray>> theta_all;
  vector<FPArray> theta(2);
  int num_partitions;
  vector<int> num_pieces, m_bits, e_bits, e_offset;
  vector<vector<vector<uint64_t>>> coeffs;
  vector<vector<uint64_t>> knots;
  if (max_ULP_error >= 64) {
    num_partitions = 3;
    num_pieces = {18, 30, 57};
    m_bits = {0, 4, 6};
    e_bits = {5, 2, 1};
    e_offset = {24, 6, 2};
    coeffs = neg_exp_imprecise_coeffs_64;
    knots = neg_exp_imprecise_knots_64;
  } else if (max_ULP_error >= 3) {
    num_partitions = 5;
    num_pieces = {28, 128, 128, 256, 197};
    m_bits = {3, 6, 7, 8, 9};
    e_bits = {5, 2, 1, 0, 0};
    e_offset = {24, 7, 3, 2, 1};
    coeffs = neg_exp_imprecise_coeffs_3;
    knots = neg_exp_imprecise_knots_3;
  } else {
    abort();
  }

  vector<FixArray> idx(num_partitions);
  theta_all.resize(num_partitions);
  for (int i = 0; i < num_partitions; i++) {
    idx[i] = get_idx_from_input(fix, delta_m, delta_e, m_bits[i], e_bits[i], e_offset[i] - delta.e_bias());
    if (i == num_partitions-1 && max_ULP_error < 64 && max_ULP_error >= 3) {
      // only need 8 bits of m and not 9
      idx[i] = fix->reduce(idx[i], 8);
    }
    theta_all[i] = fp_op->GetCoeffs(coeffs[i], knots[i], idx[i], num_pieces[i], FP_INTMD_M_BITS, FP_INTMD_E_BITS);
    assert(theta_all[i].size() == 2);
    if (i) {
      BoolArray cond = fix->LT(delta_e, delta.e_bias() - e_offset[i]);
      for (int j = 0; j < 2; j++) {
        theta[j] = fp_op->if_else(cond, theta[j], theta_all[i][j]);
      }
    } else {
      for (int j = 0; j < 2; j++) {
        theta[j] = theta_all[i][j];
      }
    }
  }

  FPArray mu_else = fp_op->mul(theta[1], delta, false);
  mu_else = fp_op->add(mu_else, theta[0], true, true, false);
  FPArray mu = fp_op->if_else(cond, mu_if, mu_else);
  ...
  N = fix->extend(N, x.e_bits + 2);
  FixArray gamma_e = fix->add(mu_e, N);
  fp_op->round_and_check(mu_m, gamma_e, FP_INTMD_M_BITS - x.m_bits);
  FPArray gamma = fp_op->input(x.party, x.size, all_0.data, all_0.data,
          mu_m.data, gamma_e.data, x.m_bits, x.e_bits);

  // Special Cases

  // Case I
  BoolArray cond3 = fp_op->GT(x, 87.33654022216796875);
  // secret-sharing zero as fp_op if_else does not support PUBLIC FPArrays
  FPArray zero = fp_op->input<double>(ALICE, x.size, 0.0, x.m_bits, x.e_bits, false);
  gamma = fp_op->if_else(cond3, zero, gamma);
...
vector<FPArray> FPMath::softmax(const vector<FPArray>& x, bool impure_bfloat16) {
  int N = x.size();
  int n = x[0].size;
  ...
  if (x[0].m_bits == BFLOAT16_M_BITS && x[0].e_bits == BFLOAT16_E_BITS && impure_bfloat16) {
  ...
  // FPArray shifted_x_flat = fp_op->sub(x_max_flat, x_flat, false, true, true);
  // FPArray e_x_flat = this->exp_neg(shifted_x_flat, 3);
...
vector<FPArray> FPMath::softmax_old(const vector<FPArray>& x) {
  int N = x.size();
  int n = x[0].size;
  int m_bits = x[0].m_bits;
  int e_bits = x[0].e_bits;
