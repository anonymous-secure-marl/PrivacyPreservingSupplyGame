...
vector < vector < FPArray > > layer1Temp = make_vector_float(ALICE, 1, 128) ;

Transpose(10, 128, layer1W, layer1WReshaped);
MatMul(1, 10, 128, layer1In, layer1WReshaped, layer1Temp);
GemmAdd(1, 128, layer1Temp, layer1b, layer1Out);
Relu2(1, 128, layer1Out, layer2In, layer1ReluHot);
vector < vector < FPArray > > layer2WReshaped = make_vector_float(ALICE, 128, 128) ;

vector < vector < FPArray > > layer2Temp = make_vector_float(ALICE, 1, 128) ;

Transpose(128, 128, layer2W, layer2WReshaped);
MatMul(1, 128, 128, layer2In, layer2WReshaped, layer2Temp);
GemmAdd(1, 128, layer2Temp, layer2b, layer2Out);
Relu2(1, 128, layer2Out, layer3In, layer2ReluHot);
vector < vector < FPArray > > layer3WReshaped = make_vector_float(ALICE, 128, 2) ;

vector < vector < FPArray > > layer3Temp = make_vector_float(ALICE, 1, 2) ;

Transpose(128, 2, layer3W, layer3WReshaped);
MatMul(1, 128, 2, layer3In, layer3WReshaped, layer3Temp);
GemmAdd(1, 2, layer3Temp, layer3b, layer3Temp);
Reassign2(1, 2, layer3Temp, layer3Out);
vector < vector < FPArray > > SigmoidVal = make_vector_float(ALICE, 1, 2) ;

Sigmoid2(1, 2, layer3Temp, SigmoidVal);
ElemWiseMul2(1, 2, SigmoidVal, actionLimHigh, fwdOut);
}
...
vector < vector < FPArray > > inpCLIENT = make_vector_float(ALICE, 1, 10) ;
...
vector < vector < FPArray > > inpSERVER = make_vector_float(ALICE, 1, 10) ;
...
vector < vector < FPArray > > actionLimHigh = make_vector_float(ALICE, 1, 2) ;
...
vector < vector < FPArray > > inp = make_vector_float(ALICE, 1, 10) ;

ElemWiseAdd2(1, 10, inpCLIENT, inpSERVER, inp);
int32_t iters = 1 ;

for (uint32_t i = 0; i < 1; i++){
vector < vector < BoolArray > > layer1ReluHot = make_vector_bool(ALICE, 1, 128) ;

vector < vector < FPArray > > layer1Out = make_vector_float(ALICE, 1, 128) ;

vector < vector < FPArray > > layer2In = make_vector_float(ALICE, 1, 128) ;

vector < vector < BoolArray > > layer2ReluHot = make_vector_bool(ALICE, 1, 128) ;

vector < vector < FPArray > > layer2Out = make_vector_float(ALICE, 1, 128) ;

vector < vector < FPArray > > layer3In = make_vector_float(ALICE, 1, 128) ;

vector < vector < FPArray > > fwdOut = make_vector_float(ALICE, 1, 2) ;

vector < vector < FPArray > > layer3Out = make_vector_float(ALICE, 1, 2) ;

vector < FPArray > loss = make_vector_float(ALICE, 1) ;

forward(layer1W, layer1b, layer2W, layer2b, layer3W, layer3b, inp, layer1ReluHot, layer1Out, layer2In, layer2ReluHot, layer2Out, layer3In, fwdOut, actionLimHigh, layer3Out);
if ((__party == ALICE)) {
