...
void test_op() {
  ...
  case Op::DOT_PRODUCT:
    ...
    fp = fp_op->dot_product(fp_1, fp_2, impure_bfloat16);
  ...
  case Op::MATMUL:
    ...
    for (int i = 0; i < n1; i++) {
      for (int j = 0; j < n3; j++) {
        ...
        for (int k = 0; k < n2; k++) {
          ...
          /* f_dbl[i*n3 + j] += (double(f_1[i*n2 + k]) * double(f_2[k*n3 + j])); */
        }
    ...
    fpm = fp_op->matrix_multiplication(fpm_1, fpm_2, impure_bfloat16);
  ...
  case Op::SOFTMAX:
    ...
    {
      vector<FPArray> fp_vec = fp_math->softmax(fp_1);
      /* vector<FPArray> fp_vec = fp_math->softmax_old(fp_1); */
