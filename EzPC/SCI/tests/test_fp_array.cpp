...
enum class Op { ADD, MUL, DIV, SQRT, CHEAP_ADD, CHEAP_DIV, SOFTFLOAT_ADD, SOFTFLOAT_MUL, SOFTFLOAT_DIV };

enum class CmpOp { LT, GT, GE, LE, SOFTFLOAT_LT };

Op op = Op::DIV;
CmpOp cmp_op = CmpOp::SOFTFLOAT_LT;
bool verbose = true;
...
void test_op() {
  ...
  case Op::CHEAP_ADD:
  case Op::CHEAP_DIV:
  case Op::SOFTFLOAT_ADD:
  case Op::SOFTFLOAT_MUL:
  case Op::SOFTFLOAT_DIV:
  ...
  for (int i = 0; i < sz; i++) {
    ...
    // f_1[i] = -3.32174454609202e-10;
    // f_2[i] = 1.17950847677203e-36;
  }
  ...
  case Op::SOFTFLOAT_ADD:
    ...
    fp = fp_op->add_softfloat(fp_1, fp_2);
    break;
  case Op::SOFTFLOAT_MUL:
    for (int i = 0; i < sz; i++) {
      f[i] = f_1[i] * f_2[i];
    }
    fp = fp_op->mul_softfloat(fp_1, fp_2);
    break;
  case Op::SOFTFLOAT_DIV:
    for (int i = 0; i < sz; i++) {
      f[i] = f_1[i] / f_2[i];
    }
    fp = fp_op->div_softfloat(fp_1, fp_2);
    break;
  default:
    assert(false);
  }
  ...
  for (int i = 0; i < sz; i++) {
    ...
    if ((f32_is_nan(f_int_1) || f32_is_denormal_number(f_int_1) ||
      ...
      continue;
    if (f32_is_denormal_number(f_int) && ! (op == Op::SOFTFLOAT_ADD || op == Op::SOFTFLOAT_MUL || op == Op::SOFTFLOAT_DIV))
      assert(f_[i] == 0.0);
    else {
      if ((op != Op::CHEAP_ADD && op != Op::CHEAP_DIV) && op != Op::SOFTFLOAT_ADD && op != Op::SOFTFLOAT_MUL && op != Op::SOFTFLOAT_DIV &&
          (m_bits == 23 && e_bits == 8)) {
        assert(f[i] == f_[i]);
      }
    }
...
void test_cmp_op() {
  ...
  case CmpOp::GE:
    ...
    break;
  case CmpOp::SOFTFLOAT_LT:
    for (int i = 0; i < sz; i++) {
      b[i] = f_1[i] < f_2[i];
    }
    bp = fp_op->LT_softfloat(fp_1, fp_2);
    break;
  default:
    assert(false);
  }
  ...
  for (int i = 0; i < sz; i++) {
    ...
    if (f32_is_nan(f_int_1) || f32_is_nan(f_int_2) || cmp_op == CmpOp::SOFTFLOAT_LT)
      continue;
    assert(b[i] == b_[i]);
  }
  delete[] b;
