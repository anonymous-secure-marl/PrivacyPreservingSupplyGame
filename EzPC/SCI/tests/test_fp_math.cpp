...
enum class Op { SINPI, COSPI, TANPI, EXP2, LOG2, EXP, LN, ERF, NEG_EXP, SIGMOID };
...
double ULP_error_bfloat16(float actual, double expected) {
  if (abs(expected) < pow(2, -126)) {
    if (actual == 0.0)
      expected = 0;
  }
...
double precise_neg_exp(float x) {
  mpfr_set_flt(mval, -1.0*x, MPFR_RNDN);
  mpfr_exp(mval, mval, MPFR_RNDN);
  return mpfr_get_d(mval, MPFR_RNDN);
}
...
double precise_sigmoid(float x) {
  mpfr_set_flt(mval, -1.0 * x, MPFR_RNDN);
  mpfr_exp(mval, mval, MPFR_RNDN);
  mpfr_add_d(mval, mval, 1.0, MPFR_RNDN);
  mpfr_d_div(mval, 1.0, mval, MPFR_RNDN);
  return mpfr_get_d(mval, MPFR_RNDN);
}
...
void test_op() {
  ...
  case Op::ERF:
    ...
    break;
  case Op::SIGMOID:
    m_bits = BFLOAT16_M_BITS;
    e_bits = BFLOAT16_E_BITS;
    // lb = pow(2.0, -8);//-1.0 * std::numeric_limits<float>::max();
    // ub = pow(2.0, 8);//std::numeric_limits<float>::max();
    lb = -1.0 * pow(2.0, 8);//-1.0 * std::numeric_limits<float>::max();
    ub = -1.0 * pow(2.0, -8);//std::numeric_limits<float>::max();
    break;
  default:
    assert(false);
  }
  ...
  case Op::NEG_EXP:
    cout << "NEG_EXP" << endl;
    for (int i = 0; i < sz; i++) {
      f[i] = precise_neg_exp(f_1[i]);
    }
    fp = fp_math->exp_neg(fp_1);
    break;
  case Op::SIGMOID:
    cout << "SIGMOID" << endl;
    // we're sampling floats which are truncted to bfloat16 inside SecFloat
    // So, the accuracy testing should be done against these truncated inputs
    // To enable this, we set f_1 to the truncated inputs underlying fp_1
    {
      FPArray fpm_1_pub = fp_math->fp_op->output(PUBLIC, fp_1);
    ...
    for (int i = 0; i < sz; i++) {
      f[i] = precise_sigmoid(f_1[i]);
    }
    if (m_bits == 23)
    fp = fp_math->sigmoid_fp32(fp_1);
    else if (m_bits == 8)
    fp = fp_math->sigmoid_bf16(fp_1);
  ...
  for (int i = 0; i < sz; i++) {
    ...
    if (op == Op::SIGMOID) ulp_err = ULP_error_bfloat16(f_[i], f[i]);
    if (verbose) {
    ...
    if (op == Op::SIGMOID) {
      assert(ULP_error_bfloat16(f_[i], f[i]) <= 0.5);
    }
    else {
